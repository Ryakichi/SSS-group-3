...
for i in range(len(conResDf)):
#for i, consistId, flowId, appId, isConsistent in conResDf.itertuples():
	consistId = conResDf.iloc[i][conResDf.columns[0]]
	flowId = conResDf.iloc[i][conResDf.columns[1]]
	appId = conResDf.iloc[i][conResDf.columns[2]]
	isConsistent = conResDf.iloc[i][conResDf.columns[3]]
	#print 'Progress: ', i, ' / ', len(conResDf.index)
