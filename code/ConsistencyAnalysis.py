...
def loadFlowResults(events_df, packageName, cdb, subsetNum):
...
#	con.init_static()
	...
	events_df = pd.read_csv('/ext/data/flows.csv', header = 0, names = column_names, dtype={'app_name': unicode, 'privacy_policy': unicode})
	...
	#for polPath in files:
		polPath = files[i]
		print 'Starting', polPath, 'Progress: ', i, ' / ', len(files)
		...
		flows = loadFlowResults(events_df, packageName, cdb, subsetNum)
